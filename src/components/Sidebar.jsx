      ...
      <div className="ml-4 mt-2 relative">
        <div className="sidebar-item flex justify-between items-center text-vscode-text">
          <div className="flex items-center cursor-pointer">
            <FontAwesomeIcon
              icon={faFolder}
              className="mr-2 text-vscode-folder"
            />
            <span className="font-bold">{item.name}</span>
          </div>
          <div className="flex items-center sidebar-icons">
            <button
              onClick={() => onCreateFolder([...path, item.name])}
              className="p-1"
            >
              <FontAwesomeIcon icon={faFolderPlus} />
            </button>
            <button
              onClick={() => onCreateFile([...path, item.name])}
              className="p-1"
            >
            ...
            {path.length ? (
              <button
                onClick={() => handleDelete([...path, item.name], "folder")}
                className="p-1"
              >
...
const Sidebar = ({ structure, isSidebarOpen }) => {
  ...
  return (
    <div
      className={`md:visible md:relative md:w-1/5 fixed top-0 left-0 h-full w-3/4  z-50 overflow-y-auto overflow-x-auto transform transition-transform ease-in-out  ${
    ...
    >
      <div className="p-4 h-full">
        ...
        <div className="h-[calc(100%-3rem)] overflow-y-auto overflow-x-auto text-white">
